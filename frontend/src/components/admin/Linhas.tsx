...
/**
 * Componente de gerenciamento de Linhas.
 * Pode ser embutido em qualquer página admin (ex.: aba dentro de Modelos)
 * ou renderizado como página standalone.
 */
...
export default function Linhas() {
  ...
  const abrirEditar = (l: Linha) => {
    setEditando(l);
    setForm({ nome: l.nome, slug: l.slug, cor: l.cor ?? COR_PADRAO, icone: l.icone ?? 'Package' });
    setSlugManual(true);
    setErro(null);
    setModal('editar');
  };
  ...
  const salvar = async () => {
    ...
    setErro(null);
    try {
      const payload = { nome: form.nome.trim(), slug: form.slug.trim(), cor: form.cor, icone: form.icone };
      if (modal === 'criar') {
  ...
  const pedirExcluir = (l: Linha) => { setErroExcluir(null); setExcluindoLinha(l); };
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h2 className="text-lg font-bold text-gray-900">Linhas</h2>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-16">
          <div className="w-7 h-7 rounded-full border-4 border-blue-200 animate-spin" style={{ borderTopColor: '#005ED5' }} />
        </div>
      ) : linhas.length === 0 ? (
        <div className="text-center py-16 text-gray-400">
          <p className="text-base font-semibold mb-2">Nenhuma linha ainda</p>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-5">
              ...
              <div className="bg-gray-50 rounded-xl p-3 flex items-center gap-3">
                ...
                >
                  {(() => { const I = iconePorNome(form.icone); return <I size={20} />; })()}
                </div>
                <div className="flex-1 min-w-0">
              ...
              <button
                onClick={salvar}
                disabled={salvando || !form.nome.trim() || !form.slug.trim()}
